...
export class PhotoService {
  ...
  protected convertDateFromClient(photo: IPhoto): IPhoto {
    return Object.assign({}, photo, {
      taken: photo.taken?.isValid() ? photo.taken.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.taken = res.body.taken ? dayjs(res.body.taken) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((photo: IPhoto) => {
        photo.taken = photo.taken ? dayjs(photo.taken) : undefined;
      });
    }
    return res;
  }
